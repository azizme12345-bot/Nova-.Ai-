...
export class TextToSpeechService {
  ...
  public unlockAudio(): void {
    ...
    try {
      ...
      if (!this.isUnlocked) {
        ...
        if (p !== undefined) {
          p.then(() => {
            if (el.src && el.src.startsWith('data:audio')) {
              el.pause();
            }
  ...
  async speak(
    ...
    // Engine 1: Direct HTML5 Audio Streaming (Preserves touch gesture activation in WhatsApp WebView, mobile Safari, and Chrome)
    try {
      ...
      // Watchdog: If audio stream takes > 3.5s to start, fallback to SpeechSynthesis
      const watchdog = setTimeout(() => {
        if (this.currentMessageId === messageId && this.isLoadingInternal) {
          console.warn('HTML5 Audio loading timed out, switching to speech synthesis');
          audio.pause();
      ...
      audio.onerror = () => {
        clearTimeout(watchdog);
        console.warn('HTML5 Audio direct stream failed, falling back to Web Speech API');
        this.speakWithSpeechSynthesis(messageId, cleanText, language, targetLang, rate, pitch, callbacks);
      };
      ...
      console.warn('Direct HTML5 Audio setup failed, using Web Speech API:', err);
      return this.speakWithSpeechSynthesis(messageId, cleanText, language, targetLang, rate, pitch, callbacks);
    }
  }
  ...
  private speakWithSpeechSynthesis(
    ...
    try {
      ...
      // Watchdog timer: ensure UI flips to speaking within 350ms even if mobile Chrome delays utterance.onstart
