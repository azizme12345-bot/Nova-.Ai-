...
export class AudioRecorderService {
  ...
  isSupported(): boolean {
    ...
    return (hasMediaDevices || hasLegacyGetUserMedia) && hasMediaRecorder;
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      recorder.onstop = async () => {
        try {
          ...
          // Determine clean mime for Gemini
          const rawMime = mimeType.split(';')[0].trim().toLowerCase() || 'audio/webm';
          ...
          reader.onloadend = async () => {
            try {
              ...
              console.warn('Transcribe request error:', postErr);
