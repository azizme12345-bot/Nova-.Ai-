...
import { WhatsAppInAppBanner } from './components/WhatsAppInAppBanner';
import { HomePage } from './components/HomePage';
import { LiveVoiceCallModal } from './components/LiveVoiceCallModal';
import { SiteRoundReportModal } from './components/SiteRoundReportModal';
...
export default function App() {
  ...
  const handleStartChatFromHome = (starterPrompt?: string) => {
    setCurrentView('chat');
    if (starterPrompt) {
  ...
  return (
    <div className="flex h-screen bg-[#0b0b0b] text-white">
      ...
      <Sidebar
        ...
        onOpenLiveCall={() => setLiveCallOpen(true)}
        onOpenSiteReport={() => setSiteReportOpen(true)}
      />

      {/* Main Chat Area */}
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        {/* Simple Top Bar */}
        <header className="h-14 flex items-center justify-between px-4 border-b border-[#222222]">
          <button className="p-2" onClick={() => setSidebarOpen(true)}>☰</button>
          <div className="flex items-center gap-2">
            <button className="text-xs text-[#a0a0a0] hover:text-white" onClick={() => setCurrentView('home')}>Home</button>
            <span className="text-sm font-medium">NOVA AI</span>
          </div>
          <button className="p-2" onClick={() => setSettingsOpen(true)}>⚙️</button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="max-w-3xl mx-auto space-y-6">
            {currentView === 'home' || messages.length === 0 ? (
                <EmptyChatState speechLang={settings.speechLang} />
            ) : (
                messages.map((msg) => (
                  <ChatMessageItem
                    key={msg.id}
      ...
      <LiveVoiceCallModal
        ...
        speechLang={settings.speechLang}
        onSendMessageToChat={(userText, aiText) => {
          if (userText && aiText) {
            handleSendMessage(userText, []);
          }
        }}
      />
      <SiteRoundReportModal
        isOpen={siteReportOpen}
        onClose={() => setSiteReportOpen(false)}
        speechLang={settings.speechLang}
        onInsertReportToChat={(reportText) => {
          handleSendMessage(reportText, []);
        }}
      />
      {ttsError && (
